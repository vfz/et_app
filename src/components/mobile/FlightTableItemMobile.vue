        ...
        <div class="row">
          <div class="col-12">
            <div class="table-item-part-left time-length-trip-block d-flex justify-content-center align-content-center">
              ...
              <span v-if="flight.time_duration_trip.split(':')[0] > 0" class="time-length-trip">
                {{ flight.time_duration_trip.split(':')[0] }}
                {{ timeFormat(flight.time_duration_trip, 'hours') }}
                &nbsp;
              </span>
              <span v-if="flight.time_duration_trip.split(':')[1] > 0" class="time-length-trip">
        ...
        <div class="row">
          ...
          <div v-if="flight.count_available_seats_trip !== 0" class="col-6">
            <div class="table-item-part-right d-flex justify-content-center flex-column h-100">
              ...
              <div v-if="flightType === 'there' || flightType === 'back'" class="table-item-part-right-date">
                <div
                  v-if="selectedSeat.filter(flightFilter => (flightFilter.id_trip === flight.id_trip || flightFilter.id_ticket === flight.ticket_id_2)).length > 0"
                  class="arrival-time table-link" @click="updatebBusTriptId(flight.id_trip)" data-bs-toggle="modal"
                  ...
                  <span>
                    {{ selectedSeat.filter(flightFilter => (flightFilter.id_trip === flight.id_trip ||
                      flightFilter.id_ticket === flight.ticket_id_2))[0].seats.toString() }}
                  </span>
                </div>
                <div v-if="+flight.count_available_seats_trip >= getAdultsCount + getChildrensCount"
